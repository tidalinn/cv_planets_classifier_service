...
class Classificator:
    ...
    def preprocess(self, image: npt.NDArray[np.int8]) -> npt.NDArray[np.int8]:
    ...
    def predict(self, image: npt.NDArray[np.int8]) -> list[str]:
    ...
    def predict_proba(self, image: npt.NDArray[np.int8]) -> dict[str, float]:
        image = self.preprocess(image)
        preds = self.session.run(None, {self.input_name: image})[0]

        return {
            tag: f'{self._sigmoid(pred):.4f}'
            for tag, pred in zip(self.classes, preds[0])
        }
    ...
    def _sigmoid(self, preds: npt.NDArray[np.int8]) -> float:
